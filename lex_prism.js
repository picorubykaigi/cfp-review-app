// ruby/prism の wasm で Ruby のソースをトークンに分ける。
//
//   import { WASI } from ".../browser_wasi_shim";
//   const lex = await createPrismLexer({ wasi: new WASI([], [], []) });
//   lex("puts 1") //=> [{ type: "IDENTIFIER", startOffset: 0, length: 4 }, ...]
//
// Copyright the picoruby.github.io authors. Licensed under the Apache License 2.0.
// https://github.com/picoruby/picoruby.github.io/blob/main/pages/r2p2/editor/tokenizer/prismLoader.js
// https://github.com/picoruby/picoruby.github.io/blob/main/pages/r2p2/editor/tokenizer/lexPrism.js
export const PRISM_WASM_URL =
  "https://cdn.jsdelivr.net/npm/@ruby/prism@1.9.0/src/prism.wasm";

// wasi は browser_wasi_shim の WASI でも node:wasi の WASI でもよい。
// wasmBinary を渡さなければ wasmUrl から取ってくる。
export async function createPrismLexer({ wasi, wasmUrl = PRISM_WASM_URL, wasmBinary }) {
  const wasm = wasmBinary
    ? await WebAssembly.compile(wasmBinary)
    : await WebAssembly.compileStreaming(fetch(wasmUrl));
  const wasiImport = wasi.wasiImport ?? wasi.getImportObject();
  ...
  return (source) => lexPrism(instance.exports, source);
}

// prism.wasm の instance をすでに持っているとき用。
